class mysql 
{
    ...
    { 
        "create-default-db":
            ...
            command => "/usr/bin/mysql -uroot -p$mysqlPassword -e 'create database `jordanlane`;'",
    ...
    { 
        "grant-default-db":
            command => "/usr/bin/mysql -uroot -p$mysqlPassword -e 'grant all on `jordanlane`.* to `root@localhost`;'",
